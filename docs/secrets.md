...
- Seafile: `vault_seafile_admin_email`, `vault_seafile_admin_password`, `vault_seafile_db_root_password`, `vault_seafile_db_username`, `vault_seafile_db_password`, `vault_seafile_jwt_private_key`, `vault_seafile_cache_password`. Recover the administrator identity from the deployed Seafile server's user list and its matching password from the password manager; recover the MariaDB root password, the Seafile database user and that user's password together from the deployed Compose environment, checking them against the database that owns the existing data. Recover the JWT signing key and the cache password from the same deployed environment. Preserve these as one deployed identity set. The administrator email needs nonempty local and domain parts; the database user follows the Immich identifier rules; the remaining four are opaque strings that must simply be present. Seafile documents a 32-character minimum for the JWT key, so recover or mint one at least that long; `generate-secrets.yml` writes 32 hexadecimal digits. **The administrator password is applied on the first run and never again.** The image writes it into `conf/admin.txt` on every start and its own `check_init_admin.py` consumes that file only while the user table is empty, so once the account exists a changed value in the vault is a silent no-op: the running account keeps the password it was created with, forever. Nothing in this repository can push a new one -- Seafile's documented remedy, `reset-admin.sh`, is interactive -- so during an incident treat this entry as the record of what the account was given at creation, not as a control over what it is now. The role's verification authenticates as this pair and fails by name when the two have diverged. The same file is why Seafile's `conf/` directory is on the runtime plaintext list in the security boundary: the plaintext password materialises inside the bind-mounted volume at every container start, and a container killed mid-start leaves it there. The administrator password is also spent in clear on the wire, which is a property of the platform rather than of Seafile: nothing here terminates TLS, so Seafile is published as plain HTTP and that password -- along with every user's session cookie and the file content itself -- is readable to anything that can observe the traffic. A client arriving over the mesh VPN is already encrypted by that transport; a client on the LAN is not. Treat a Seafile login made across a network you do not trust as a disclosed password, and recover this entry accordingly. There is no Postgres here; Seafile has no Postgres backend, and its own setup creates the three databases and the database user from these values on the first run.
...
- Seerr's `settings.json` and the `settings.old.json` it keeps beside it, both
  ...
  VAPID private key, and once reconciled the Radarr and Sonarr keys and the ntfy
  publisher token. The Jellyfin token is the one worth naming: Seerr's access to
