...
- Usenet provider: `vault_downloaders_sabnzbd_server_username`, `vault_downloaders_sabnzbd_server_password`. This is the account of a paid third-party Usenet subscription. It is never generated: recover it from the provider's own portal or the password manager, and preserve it unchanged. It is also the one entry here that may be absent: with no such subscription both are left empty and SABnzbd starts with no server, which `inventory/group_vars/all/main.yml` declares as the default. Both empty or both recovered — a half-recovered account is refused field by field, and a recovery that finds neither has nothing to do here. Nothing can read the password back out of the deployed service, because SABnzbd answers `get_config` with ten asterisks in its place, which is why this entry is the only record of it. The rest of the provider is not a credential and is not recovered from here: the host, port, connection count and TLS flag are operator policy in `media_usenet_provider` in `inventory/group_vars/all/main.yml`, in plain sight and in version control, with the rules SABnzbd's normalization imposes on them enforced by `filter_plugins/media_usenet_provider.py` and described in [media acquisition phase 1](media-acquisition-phase1.md). A recovery that restores this vault entry and finds an empty host has restored half a provider, and the downloaders role will say so. The platform declares this server and reverts a server edited in SABnzbd's web interface, so a change made there is lost at the next run, with one exception. A password changed in the web interface is invisible to the platform, because SABnzbd answers with asterisks in place of it and there is nothing to compare; it stands until this vault value changes. Rotate the provider password here, not there.
...
a dated record rather than a claim about the pin in
`services/beszel/compose.yml` -- so an existing unverified identity fails with
credential-migration guidance and is never auto-verified. A managed identity
cannot duplicate either the Beszel superuser or the existing primary application
user.
...
- Configuration this platform seeds once and then leaves to the application,
  ...
  and, in its `[servers]` section, the Usenet provider's username and password.
  SABnzbd writes that password back out decoded, so it is cleartext in the file
  whatever characters it contains, verified against the pinned 5.1.2 image. The
  same file's `logs/sabnzbd.log` carries it too whenever SABnzbd's log level is
  set to Debug, because its request log prints every API parameter; treat a
  debug-level log directory as secret-bearing and do not attach one to an issue; `radarr/config/config.xml`, `sonarr/config/config.xml` and
...
- Trailarr's `/config/.env`, which belongs in this class rather than merely in
  ...
  a login changed in the web interface adds the administrator name and bcrypt
  hash beside it. The platform owns the file and keeps it at mode 0600.
- Seerr's `settings.json` and the `settings.old.json` it keeps beside it, both
  written mode 0644 in its configuration root. They hold the Seerr API key, the
  Jellyfin token Seerr minted for itself at bootstrap, the session secret, the
  VAPID private key, and once reconciled the Radarr and Sonarr keys and the
  Pushover user key and Media application token. The user key is not Seerr's
  own: it is the one every Pushover application on this platform delivers to,
  so a read of either file pairs it with a token that sends to the household's
...
service lives in a file of its own named for that account: the Pushover user key
and application tokens in `inventory/group_vars/all/vault_pushover.yml`, the
healthchecks.io pair in `inventory/group_vars/all/vault_healthchecks.yml`. A new
such name is added beside theirs to the exceptions `tests/policy_vault_test.rb`
admits, because every other file there must name a service role. A service or
account with no vault file yet gets one
...
keys and its managed-user list, and the Pushover user key and the four
application tokens that Beszel, Dozzle, the deployment reports, the poller, the image prune and Seerr read
have a file of their own, as does the healthchecks.io pair the deployment
poller reports to.
...
More files in the same directory are secret-bearing. The installer renders one
protected curl config per Pushover application for the poller,
`$HOME/.config/nas-platform/pushover-alerts.curl` and `pushover-deployments.curl`,
and another pair for the weekly image prune, `pushover-prune-alerts.curl` and
`pushover-prune-containers.curl`. Each is mode 0600, is rendered under `no_log`,
...
and the task that renders it sets `no_log`.
