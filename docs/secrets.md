...
- Pushover: `vault_pushover_token`, `vault_pushover_user_key`. This is an account at a third party, pushover.net, and neither value is generated here. Recover the user key from that account's dashboard, where it is displayed, and the application token from the application's own page under that account; the password manager is the other place to look, and a token that cannot be found there can be regenerated from the application page at the cost of every client configured with the old one. Preserve the pair unchanged otherwise. Both are 30-character opaque strings. The credential contract checks only that each is present and is not the documented stand-in, because this repository cannot vouch for the format Pushover issues and a guessed shape rule would refuse a real pair on every five-minute poller tick, with the fix locked inside the encrypted vault. The stand-ins in the example file and in the generated plaintext are refused for the reason the OpenSubtitles and ComicVine ones are: a Beszel or an alert relay holding one sends alerts Pushover rejects, and nothing on this platform observes the rejection. This pair belongs to no single service — `roles/beszel` builds its notification webhook from the two, `roles/dozzle` renders both into the alert relay's environment, which POSTs them as Pushover's `token` and `user` form fields, the deployment reports and run summary in `roles/ntfy` POST them the same way from the controller, and `roles/seerr` stores both in Seerr's own Pushover agent, which sends request events from inside that container — which is why it is pinned in `GLOBAL_VAULT_KEYS` in `tests/policy_support.rb` rather than in a per-service expectations file. Losing the pair therefore silences the host-level threshold alerts, every container alert, every deployment report and every request event, and a pair Pushover refuses fails the converge at the first recreated service's report; regenerating the application token from pushover.net and converging is the whole recovery, since nothing here is read back from either consumer.
- Healthchecks: `vault_healthchecks_poller_ping_url`, `vault_healthchecks_verify_ping_url`. This is an account at a third party, healthchecks.io, and neither value is generated here. Each is the ping URL of one check under that account, shown on that check's own page: the first is the deployment poller's tick heartbeat, a check with a period of 5 minutes and a grace of 15; the second is the hourly verify verdict, a period of 1 hour and a grace of 2. Recover them from the account or the password manager; a check whose URL cannot be recovered is replaced by creating a new one with the same period and grace and converging. The token in each path is that check's whole authentication, so anyone holding it can mark the check up or down, which is why the rendered poller configuration carrying both is secret-bearing. The credential contract checks only that each is an https URL and that the two differ -- the documented stand-ins are not URLs, so that rule is what refuses them: healthchecks.io also serves slug paths and custom ping domains, and a guessed host or path rule would refuse a real URL on every five-minute poller tick, with the fix locked inside the encrypted vault. They must differ because one URL in both places collapses the tick heartbeat into the verify verdict, and every tick would then vouch for a verify that had stopped running. The pair belongs to no service -- `roles/production_auto_deploy` renders both into `deployer.json` -- so it is pinned in `GLOBAL_VAULT_KEYS` in `tests/policy_support.rb`, beside the Pushover pair. Losing it costs only the external alerts: the poller reads a missing or unusable URL as no ping, never as a reason to stop deploying, and both checks then alert on that silence.
...
- Seerr's `settings.json` and the `settings.old.json` it keeps beside it, both
  ...
  Pushover pair. That pair is not Seerr's own: it is the one Beszel, the Dozzle
  relay and the deployment reports use, so a read of either file reaches all of
  them (see the Pushover entry above). The Jellyfin token is the one worth naming: Seerr's access to
...
keys and its managed-user list, and the Pushover pair that Beszel, Dozzle, the
deployment reports and Seerr read has a file of its own, as does the healthchecks.io
pair the deployment poller reports to.
