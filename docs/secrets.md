...
- Usenet provider: `vault_downloaders_sabnzbd_server_host`, `vault_downloaders_sabnzbd_server_port`, `vault_downloaders_sabnzbd_server_username`, `vault_downloaders_sabnzbd_server_password`, `vault_downloaders_sabnzbd_server_connections`, `vault_downloaders_sabnzbd_server_ssl`. This is the account of a paid third-party Usenet subscription. It is never generated: recover it from the provider's own portal or the password manager, and preserve it unchanged. It is also the one entry here that may be absent: with no such subscription, all six are left empty and SABnzbd starts with no server, which `inventory/group_vars/all/main.yml` declares as the default. All six empty or all six recovered — a half-recovered provider is refused field by field, and a recovery that finds none of them has nothing to do here. Nothing can read the password back out of the deployed service, because SABnzbd answers `get_config` with ten asterisks in its place, which is why this entry is the only record of it. The host is a bare lowercase hostname with no scheme, the port is 1-65535 (563 for TLS, 119 without), the connection count is 1-500 and must not exceed what the subscription allows, and the TLS flag is `1` or `0`, because SABnzbd parses a server flag with `int()`: `true` is stored as 0 and silently disables TLS. The platform declares this server and reverts a server edited in SABnzbd's web interface, so a change made there is lost at the next run, with one exception. A password changed in the web interface is invisible to the platform, because SABnzbd answers with asterisks in place of it and there is nothing to compare; it stands until this vault value changes. Rotate the provider password here, not there.
