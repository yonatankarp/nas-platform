...
# upstream: https://github.com/Casvt/Kapowarr/blob/V1.3.2/backend/features/tasks.py
# upstream_sha256: e1622b14862c8e8338e458f5e931b20acce4e49087822a2072304da95b9809f0
# image: docker.io/mrcas/kapowarr:v1.3.2@sha256:cd3f004caff59636dde73b06df5b2f8068b485b47ff5fe82f6129562c0f2ac3b
#
# Everything above this trailer is that upstream file, which is byte-identical to
# /app/backend/features/tasks.py in the image above, with two lines changed and
# none added, so every traceback still names upstream's line numbers:
#
#   284  if self.queue[0]['thread'].is_alive(): self.queue[0]['thread'].join()
#   365  if task['thread'].is_alive(): task['thread'].join()
#
# Upstream joins a queued task's thread without asking whether it was ever
# started, and CPython refuses that with "cannot join thread before it is
# started". At 284 that is stop_handle() on the shutdown path: the raise aborts
# Kapowarr.py's finally before ExternalClients.disconnect_clients() and exit(),
# which on the NAS left the container to Docker's SIGKILL. At 365 it is remove(),
# which raises for every queued task that is not the running head. The guard is
# the one download_queue.py already applies to its own threads. A finished
# thread needs no join either, so is_alive() is the whole condition.
#
# services/kapowarr/compose.yml mounts this file read-only over the image's own,
# and tests/contracts/kapowarr-static.rb refuses the tree unless the image line
# above equals the Compose pin and reverting the two lines reproduces
# upstream_sha256. A Renovate bump therefore reds CI until this file is derived
# again from the new image's tasks.py, or deleted with its mount once upstream
# ships the guard.
