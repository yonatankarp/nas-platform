...
def _inspect_invoked_source():
    invoked_path = Path(os.path.abspath(__file__))
    current = Path(invoked_path.anchor)
    components = invoked_path.parts[1:]
    try:
        for index, component in enumerate(components):
            current = current / component
            metadata = current.lstat()
            if stat.S_ISLNK(metadata.st_mode):
                raise MigrationError("migration source path must not contain symlinks")
            if index < len(components) - 1:
                if not stat.S_ISDIR(metadata.st_mode):
                    raise MigrationError("migration source parent must be a directory")
            elif not stat.S_ISREG(metadata.st_mode):
                raise MigrationError("migration source must be a regular file")
    except OSError as exc:
        raise MigrationError("migration source cannot be inspected") from exc
    return invoked_path


LEXICAL_SOURCE_PATH = Path(os.path.abspath(__file__))
try:
    SOURCE_PATH = _inspect_invoked_source().resolve()
    SOURCE_INSPECTION_ERROR = None
except MigrationError as exc:
    SOURCE_PATH = LEXICAL_SOURCE_PATH
    SOURCE_INSPECTION_ERROR = str(exc)
REPOSITORY_ROOT = SOURCE_PATH.parents[1]
EXPECTED_SOURCE_PATH = REPOSITORY_ROOT / "scripts/migrate-media-acquisition-vault.py"
...
def migrate(vault_argument, password_argument):
    ...
    document = None
    try:
        if SOURCE_INSPECTION_ERROR is not None:
            raise MigrationError(SOURCE_INSPECTION_ERROR)
        if LEXICAL_SOURCE_PATH != EXPECTED_SOURCE_PATH:
            raise MigrationError("migration source is not the exact production source")
...
def main(argv=None):
    try:
        ...
        if (len(arguments) != 4 or arguments[0] != "--vault" or
                arguments[2] != "--vault-password-file"):
            raise MigrationError(
                "expected exact --vault and --vault-password-file arguments"
            )
        migrate(arguments[1], arguments[3])
    except MigrationError as exc:
