...
required_tasks = [
  ...
  "Refuse duplicate managed Komga libraries",
...
fail_contract("managed library root differs") unless library.fetch("root") == LIBRARY_ROOT
if MODE == "drift-verify"
  fail_contract("Komga drift fixture was not installed") unless library.fetch("scanOnStartup") == true
...
if MODE == "drift"
  request(
    ...
    body: { "scanOnStartup" => true }, expected: [204]
  )
...
canonical = JSON.generate(library.reject { |key, _value| %w[id unavailable].include?(key) }.sort.to_h)
case MODE
when "seed"
  ...
  puts "Komga fixture and library settings seeded"
when "assert-persistence"
  ...
  fail_contract("Komga library settings changed across recreation") unless STATE_PATH.binread == canonical
  puts "Komga library settings and scanned comic persisted"
end
RUBY
