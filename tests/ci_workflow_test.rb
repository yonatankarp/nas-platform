...
def validate_workflow(source)
  ...
  fail_contract("requires the static job") unless key?(jobs, "static")
  job = mapping(value_for(jobs, "static"), "static job")
  expected_condition = "${{ needs.changes.outputs.static == 'true' }}"
  fail_contract("static job selector changed") unless value_for(job, "if") == expected_condition
  steps = array(value_for(job, "steps"), "static steps")
  fail_contract("static steps must contain only mappings") unless steps.all? { |step| step.is_a?(Hash) }
...
def self_test
  ...
  assert_failure("steps shape", static_shape, "static steps must be a list")
