...
# `cap_drop: [ALL]` is required of every container, and the exceptions are named
# here rather than inferred, so a *new* service cannot reach the platform
# without either dropping capabilities or saying in this file why it does not.
# That is the inverse of an allowlist of the compliant, which would have gone
# quiet the moment someone added the thirty-third container.
#
# Why this is not one sweep: dropping a capability an image genuinely needs
# breaks it, sometimes without a single log line. The containers below start as
# root and reach their service account themselves, so each one's add-back list
# has to be derived by measurement before its key can land. Issue #196 records
# the instruments; the reasons here are what each container is still waiting on.
#
# A container that drops capabilities must NOT appear below. That coupling is
# deliberate: it makes a tranche remove its names in the same commit that adds
# its keys, so this list cannot rot into a description of a past state.
CAPABILITY_DROP_PENDING = {
  # The linuxserver.io s6 supervision tree was measured holding uid 0 and the
  # full default capability set for the container's entire life, not merely
  # during startup, so ablation over the paths a lane happens to take does not
  # bound what these need. s6-log rotation, an s6-svc restart after a crash and
  # a restart against an already-dirty /config are all unexercised.
  "arr/radarr" => "s6 tree holds root capabilities for the container's life",
  "arr/sonarr" => "s6 tree holds root capabilities for the container's life",
  "arr/prowlarr" => "s6 tree holds root capabilities for the container's life",
  "arr/bazarr" => "s6 tree holds root capabilities for the container's life",
  "downloaders/sabnzbd" => "s6 tree holds root capabilities for the container's life",
  # haproxy was measured running as uid 0 with the full set for the container's
  # life. It performs no privilege drop at all, so the s6 add-back list above
  # does not describe it and its own set has to be derived separately.
  "beszel/socket-proxy" => "haproxy runs as root; add-back not yet derived",
  "dozzle/socket-proxy" => "haproxy runs as root; add-back not yet derived",
  # These leave no capability holder behind once started, so their need is
  # confined to the entrypoint window that every start exercises identically.
  # That is a good position to derive from, and the derivation is still owed.
  "immich/redis" => "entrypoint privilege drop; add-back not yet derived",
  "immich/database" => "entrypoint privilege drop; add-back not yet derived",
  "paperless-ngx/broker" => "entrypoint privilege drop; add-back not yet derived",
  "paperless-ngx/db" => "entrypoint privilege drop; add-back not yet derived",
  "kapowarr/kapowarr" => "bespoke PUID entrypoint; add-back not yet derived",
  "trailarr/trailarr" => "bespoke PUID entrypoint; add-back not yet derived",
  # No residency measurement exists for any of these. Beszel's hub is distroless
  # with no shell to probe and its agents would not start outside the platform;
  # the Immich pair and the Paperless webserver need their stack's dependencies
  # to boot far enough to measure. Guessing here would aim at a live NAS.
  "beszel/hub" => "no residency measurement; distroless, cannot be probed",
  "beszel/agent-portable" => "no residency measurement; reads host telemetry",
  "beszel/agent-intel" => "no residency measurement; reads host telemetry",
  "immich/immich-server" => "no residency measurement; runs as root",
  "immich/immich-machine-learning" => "no residency measurement; runs as root",
  "paperless-ngx/webserver" => "no residency measurement; runs as root",
  # Jellyfin's image scanned clean and its PID 1 is unprivileged, so it belongs
  # with the proved-safe set on the evidence. It is held back for a different
  # reason: `devices: !override []` in both platform overrides means no lane
  # runs it with the render device it has in production.
  "jellyfin/jellyfin" => "no lane runs its production device configuration"
}.freeze

# Add-backs, pinned per container with the capability each one buys, the way
# EXPECTED_CONTAINER_CPUS pins ceilings. Empty today because the containers that
# drop capabilities are exactly the ones that need none: an unprivileged PID 1
# has nothing to add back to. A tranche that needs one states it here, so the
# set can never widen without the diff saying so.
CAPABILITY_ADD_BACK = {
  # The platform's one pre-existing add-back, which predates this check and was
  # pinned nowhere until it. The Intel agent reads GPU performance counters off
  # the render device, and perf_event_open is gated on PERFMON. It is recorded
  # here so that a second capability appearing beside it has to say why.
  "beszel/agent-intel" => ["CAP_PERFMON"]
}.freeze

# The per-container checks below catch a container that omits the key. They
# cannot catch the opposite rot: a name left in CAPABILITY_DROP_PENDING after
# the container it exempts was renamed or removed, which would sit there
# exempting nothing and quietly overstate how much is still pending.
#
# The floor is here for the reason this repository keeps relearning. Both of
# these subject lists are derived by walking the Compose files, so a glob that
# stops matching, or an edit that drops the keys, empties them and every
# `all?` over them passes vacuously. Asserting a count, not just emptiness, is
# what makes that failure loud. Twelve is the tranche that landed with the
# check; it only ever grows.
all_container_labels = service_dirs.flat_map do |dir|
  compose_path = File.join(dir, "compose.yml")
  next [] unless File.file?(compose_path)

  stack = File.basename(dir)
  YAML.safe_load_file(compose_path, aliases: true)
      .fetch("services", {}).keys.map { |container| "#{stack}/#{container}" }
end
dropping_containers = all_container_labels - CAPABILITY_DROP_PENDING.keys
stale_pending = CAPABILITY_DROP_PENDING.keys - all_container_labels
check(failures, stale_pending.empty?,
      "CAPABILITY_DROP_PENDING names containers that do not exist: #{stale_pending.sort.join(', ')}")
check(failures, all_container_labels.length >= 32,
      "capability policy must cover every container; found only #{all_container_labels.length}")
check(failures, dropping_containers.length >= 12,
      "at least twelve containers must drop capabilities; found #{dropping_containers.length}")
check(failures, CAPABILITY_DROP_PENDING.values.all? { |reason| reason.is_a?(String) && !reason.empty? },
      "every CAPABILITY_DROP_PENDING entry must record why that container is still pending")
...
service_dirs.each do |dir|
  ...
  containers.each do |container, spec|
    ...
    # The third part of the same boundary, and the one no_new_privs does not
    # cover. Refusing `privileged` says the container starts without extra
    # power and no_new_privs says it cannot acquire any by executing a setuid
    # binary; neither touches the capabilities Docker hands out by default.
    # no_new_privs specifically does NOT neutralise a file capability: a binary
    # carrying `+ep` still execs and still gets its caps under no_new_privs,
    # and only an empty bounding set refuses it. So this key is doing work the
    # other two cannot, which is why it is checked separately rather than
    # folded into the shared fragment.
    #
    # It is checked here, per container, rather than only on x-service-defaults,
    # because services/bindery/compose.yml declares no such fragment: a check
    # that lived only on the anchor would exempt bindery silently.
    pending_capability_reason = CAPABILITY_DROP_PENDING[label]
    if pending_capability_reason
      check(failures, !spec.key?("cap_drop"),
            "#{label}: drops capabilities now, so it must be removed from " \
            "CAPABILITY_DROP_PENDING in the same commit")
    else
      check(failures, Array(spec["cap_drop"]) == ["ALL"],
            "#{label}: must drop Docker's default capabilities with " \
            "cap_drop: [ALL], or be named in CAPABILITY_DROP_PENDING with a reason")
    end
    expected_add_back = CAPABILITY_ADD_BACK.fetch(label, [])
    check(failures, Array(spec["cap_add"]).sort == expected_add_back,
          "#{label}: cap_add must be the pinned add-back set " \
          "#{expected_add_back.inspect}, each capability recorded with its reason")
