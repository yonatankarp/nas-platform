...
def initialize_input(path, options)
  input = {
    ...
    "vaultwarden_port" => options.fetch(:vaultwarden_port),
    "karakeep_port" => options.fetch(:karakeep_port),
...
parser = OptionParser.new do |opts|
  ...
  opts.on("--vaultwarden-port PORT", Integer) { |value| options[:vaultwarden_port] = value }
  opts.on("--karakeep-port PORT", Integer) { |value| options[:karakeep_port] = value }
