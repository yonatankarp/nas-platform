...
adguard=its contract declares only static and run, so there is no seed phase to dispatch; the whole of its configuration is one file the role templates before the daemon starts for the first time, and the only state a fixture could add is the filter lists it downloads for itself -- which its own run phase polls for rather than seeding
