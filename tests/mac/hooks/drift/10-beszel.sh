...
# Read against Beszel 0.18.7 (a dated record; services/beszel/compose.yml
# carries the pin), the hub exposes system_stats and container_stats as
# authenticated, read-only collections (no supported create/update/delete rule).
# Editing the PocketBase database is explicitly unsupported, and stopping real
# metrics would destructively weaken the live proof. Exercise telemetry-fixtures
# through the same category evaluator instead of faking a live telemetry repair.
