...
expected_failure=$(mktemp "$PLATFORM_REPORT_ROOT/paperless-verify-drift.XXXXXX")
trap 'unlink "$expected_failure" >/dev/null 2>&1 || true' EXIT HUP INT TERM

"$mac_script_dir/run-paperless-contract.sh" drift
"$mac_script_dir/run-paperless-contract.sh" drift-verify
if mac_ansible_playbook -i "$mac_repo_dir/inventory/mac.yml" "$mac_repo_dir/verify.yml" \
    ...
    --tags platform_verify_paperless >"$expected_failure" 2>&1; then
  printf '%s\n' 'verification-only run accepted Paperless drift' >&2
  exit 1
fi
"$mac_repo_dir/tests/assert-no-vault-secrets.rb" \
  "$PLATFORM_MAC_VAULT_FILE" "$PLATFORM_MAC_VAULT_PASSWORD_FILE" "$expected_failure"
grep -qF 'managed Paperless administrator, Gmail account, or mail rule is absent or drifted' \
  "$expected_failure"
